...
interface DynamicGridTabsProps {
  activeTab: string;
  onTabChange: (tab: string) => void;
}

const TABS = [
  { id: 'associate', label: 'Associate Slabs', icon: Users },
  { id: 'tl', label: 'Team Leader (TL) Slabs', icon: UserCheck },
  { id: 'am', label: 'Assistant Manager (AM) Slabs', icon: Award },
  { id: 'riders', label: 'Riders & Docking Rules', icon: ShieldAlert },
];

export function DynamicGridTabs({ activeTab, onTabChange }: DynamicGridTabsProps) {
  return (
      ...
      {TABS.map((tab) => {
        ...
        return (
          <button
            ...
            className={`flex items-center gap-2.5 px-3.5 py-2 text-xs transition-all whitespace-nowrap cursor-pointer flex-1 min-w-[170px] border rounded-none ${
              isActive

...
import { Settings, Trash2, Building2, MapPin } from 'lucide-react';
  ...
  return (
    <div className="border border-slate-200 bg-white shadow-2xs overflow-hidden flex flex-col flex-1 rounded-none">
      <div className="px-4 py-2.5 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
        <h3 className="font-bold text-xs text-slate-800 uppercase tracking-wider">Configured Business Processes</h3>
      ...
      <table className="w-full text-xs text-left">
        <thead className="bg-slate-50/60 text-slate-500 border-b border-slate-200">
          <tr>
            ...
            <th className="px-4 py-2 font-semibold">Process Name</th>
            <th className="px-4 py-2 font-semibold">Client</th>
            <th className="px-4 py-2 font-semibold">Location</th>
            <th className="px-4 py-2 font-semibold">Process Head</th>
          ...
          {loading ? (
            ...
            <tr>
              <td colSpan={6} className="px-5 py-12 text-center text-slate-500">
                <Settings className="mx-auto h-6 w-6 text-slate-300 mb-2" />
                <p className="font-semibold text-slate-800">No processes found</p>
                <p className="text-[11px] text-slate-400 mt-0.5">{searchQuery ? 'Try adjusting your search query' : 'Click "Add Process" to create one.'}</p>
              </td>
            </tr>
          ) : (

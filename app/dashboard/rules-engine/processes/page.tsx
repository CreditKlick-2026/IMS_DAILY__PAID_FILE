...
export default function ProcessesPage() {
  ...
  const [trackingMetric, setTrackingMetric] = useState('Amount'); // Default to Amount
  ...
  const handleAdd = async () => {
    if (!name.trim() || !clientId || !locationId || !trackingMetric) {
      alert("Please fill in all fields (Client, Location, Process Name, Tracking Metric).");
      return;
    }
      ...
      {addMode && (
        <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-5 shadow-sm animate-in fade-in slide-in-from-top-4 duration-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            ...
            <div>
              <label className="block text-xs font-bold text-indigo-900 mb-1 uppercase tracking-wider">Tracking Metric</label>
              <select 
                className="w-full px-3 py-2 border border-indigo-200 rounded-md outline-none focus:ring-2 focus:ring-indigo-500 bg-white"
                value={trackingMetric}
                onChange={(e) => setTrackingMetric(e.target.value)}
              >
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50/50 text-slate-500 border-b">
              <tr>
                ...
                <th className="px-5 py-3 font-medium">Tracking Metric</th>

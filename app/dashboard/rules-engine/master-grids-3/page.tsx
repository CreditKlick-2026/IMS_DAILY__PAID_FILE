...
export default function MasterGrid3Page() {
  const [data, setData] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch('/api/admin/master-grids-3')
      .then(res => res.json())
      .then(json => {
        setData(json.data);
        setLoading(false);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Master Grid 3 — SBI Recovery Slabs</h1>
        <p className="text-gray-500 mt-1">Configure PCP-based tiers for Associates, TLs, and AMs</p>
      </div>

      <div className="bg-white rounded-lg shadow border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
          <h3 className="font-semibold text-gray-900">Associate Slabs (PCP based)</h3>
        </div>
